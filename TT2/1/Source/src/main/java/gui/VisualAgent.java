...
public class VisualAgent {
	...
	public VisualAgent(Agent agent, String autonomousAgentSpriteSheet, String interactiveAgentSpriteSheet){
		this.agent = agent;
		
		try {
			if(agent.isInteractive())
			
				spriteSheet = new SpriteSheet(interactiveAgentSpriteSheet, agent.getWidth(), agent.getHeight(), 0);
			else
				spriteSheet = new SpriteSheet(autonomousAgentSpriteSheet, agent.getWidth(), agent.getHeight(), 0);
